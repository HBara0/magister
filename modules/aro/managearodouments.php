...
if(!($core->input['action'])) {
    ...
    $consolidation_warehouses['0'] = $lang->nowarehouse;
    ksort($consolidation_warehouses);
...
//                        }
            ...
            if($aroorderrequest->revision > 0) {
                $revisiedaro_output = ' <div class="panel panel-warning" style="width:15%;padding:5px;"><strong>'.$lang->timesmodified.'</strong>: '.$aroorderrequest->revision_output.'</div>';
            }
                    ...
                    foreach($periods as $period) {
                        ...
                        if($purchasetype->isPurchasedByEndUser == 1) {
                            if(is_object($RIC_customer_obj)) {
                                ...
                                $extrafilter = ' AND c.cid='.$RIC_customer_obj->eid;
                            }
                        }
                        $amount[$period] = $totalfunds->get_fundsamount($uptodate, $aroorderrequest->affid, array('join' => $extrajoin, 'filter' => $extrafilter));
                        foreach($totalfunds_fiels as $totalfunds_field) {
                            $fundsengaged_evolution_row[$totalfunds_field] .= '<td class="border_right" style="text-align:right;">'.$numfmt->formatCurrency(round(($amount[$period][$totalfunds_field]) / 1000), "USD").'</td>';
                            $trend[$totalfunds_field][$period] = round($amount[$period][$totalfunds_field] / 1000);
                        }
                    }
                    foreach($totalfunds_fiels as $totalfunds_field) {
...
//        }
    ...
    if(isset($core->input['referrer']) && $core->input['referrer'] == 'toapprove') {
        if(is_object($aroordersummary)) {
            ...
            $ordersummary_fields = array('netmarginIntermed_afterdeduction', 'invoiceValueIntermed', 'invoiceValueLocal', 'invoiceValueUsdIntermed', 'invoiceValueUsdLocal', 'interestValue', 'interestValueUsd', 'totalIntermedFees', 'totalIntermedFeesUsd', 'unitFee', 'netmarginIntermed', 'netmarginLocal', 'invoiceValueThirdParty', 'globalNetmargin', 'totalQuantityUom', 'riskRatioAmount'); // 'netmarginIntermedPerc', 'netmarginLocalPerc');
    ...
    if($core->input['action'] == 'populateproductlinefields') {
        ...
        $parmsfornetmargin = array('localPeriodOfInterest', 'localBankInterestRate', 'warehousingPeriod', 'warehousingTotalLoad', 'warehousingRate', 'intermedBankInterestRate', 'intermedPeriodOfInterest', 'commission', 'totalDiscount', 'totalQty', 'localRiskRatio');
