...
if(!$core->input['action']) {
	if($core->input['type'] == 'addmenuitem') {
		...
		/* Parse lists needed for menu configurations - START */
		$affiliates = get_specificdata('affiliates', array('affid', 'name'), 'affid', 'name', array('by' => 'name', 'sort' => 'ASC'), 1);
		$list_affiliates = parse_selectlist('menuitem[configurations][affiliate][]', 1, $affiliates, array(0=>''), 1);
		...
		$webpages = get_specificdata('cms_pages', array('title', 'title'), 'title', 'title', array('by' => 'title', 'sort' => 'ASC'), 1, 'isPublished = 1');
		$list_webpages = parse_selectlist('menuitem[configurations][webpages]', 1, $webpages,  0);
		/* Parse lists needed for menu configurations - END */
