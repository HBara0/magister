 ...
 */
	...
	if(is_array($multiple_meetings)) {
		foreach($multiple_meetings as $mid => $meeting) {
			$setmeeting_tools = $edit_tool = '';
			if($meeting['createdBy'] == $core->user['uid']) {

				if($meeting[hasMoM] == 1) {
					$action = '&do=edit';
				}
				$edit_tool = '<a href=index.php?module=meetings/create&mtid='.$meeting['mtid'].' title="'.$lang->edit.'"><img src=./images/icons/edit.gif border=0 alt='.$lang->edit.'/></a> ';
				$setmeeting_tools = ' <a href=index.php?module=meetings/minutesmeeting'.$action.'&referrer=list&mtid='.$meeting['mtid'].' title="'.$lang->setmof.'" rel="setmof_'.$meeting['mtid'].'"><img src="'.$core->settings['rootdir'].'/images/icons/mof.png" alt="'.$lang->delete.'" border="0"></a>   ';
			}
