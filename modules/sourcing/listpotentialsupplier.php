 ...
 * Last Update: 	@tony.assaad		November 12, 2012 | 4:13 PM
 */
...
//	if(!$core->input['action']) {
	...
	$chemicals_query = $db->query("SELECT csid,casNum,name from chemicalsubstances ORDER BY name ASC");
	...
	if(is_array($potential_suppliers)) {
		...
		if($core->usergdroup['sourcing_canListSuppliers'] == 0) {
			$chemicals_query = $db->query("SELECT csid,casNum,name from chemicalsubstances ORDER BY name ASC");
			while($chemicals = $db->fetch_assoc($chemicals_query)) {
				$chemical_list.='<option value='.$chemicals['csid'].'>'.$chemicals['casNum'].'-'.$chemicals['name'].'</option>';
			}

			//	eval("\$sourcing_listpotentialsupplier_rows = \"".$template->get('sourcing_popuprequestchemical')."\";");
			$sourcing_listpotentialsupplier_rows .= '<tr><td colspan="5"><a href="#" id="showpopup_requestchemical" class="showpopup"><img alt="Add" src="./images/addnew.png" border="0"/>request chemical'.$lang->createmenu.'</a></td></tr>';
		}
	}
	//} 
