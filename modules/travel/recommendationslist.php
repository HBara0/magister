...
if (!isset($core->input['action'])) {
    ...
    if (is_array($recommendation_objs)) {
        foreach ($recommendation_objs as $recommendation_obj) {
            ...
            $tool_items = ' <li><a target="_blank" href="' . $recommendation_obj->get_link() . '"><span class="glyphicon glyphicon-eye-open"></span>&nbsp' . $lang->viewcourse . '</a></li>';
            if ($recommendation_obj->canManageRecommendation()) {
                $tool_items .= ' <li><a target="_blank" href="' . $recommendation_obj->get_editlink() . '"><span class="glyphicon glyphicon-pencil"></span>&nbsp' . $lang->managecourse . '</a></li>';
            }
