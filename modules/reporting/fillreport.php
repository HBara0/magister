 ...
 * $module: reporting
 * $id: fillreport.php
 * Last Update: @zaher.reda 	July 17, 2012 | 03:55 PM
 */
...
if(!$core->input['action']) {
	...
	if($core->input['stage'] == 'productsactivity') {
		...
		$currencies_from = parse_date('j/m/Y', $core->settings['q'.$core->input['quarter'].'start'].'/'.$core->input['year']); // date_timestamp_get(date_create_from_format('j/m/Y', $core->settings['q'.$core->input['quarter'].'start'].'/'.$core->input['year']));
		$currencies_to = parse_date('j/m/Y', $core->settings['q'.$core->input['quarter'].'end'].'/'.$core->input['year']); // date_timestamp_get(date_create_from_format('j/m/Y', $core->settings['q'.$core->input['quarter'].'end'].'/'.$core->input['year']));
