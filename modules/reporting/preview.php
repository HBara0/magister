...
if(!$core->input['action']) {
	...
	$productsdata_perquarter = array();
		...
		if(!empty($productsdata)) {
			...
			foreach($productsdata_perquarter as $key => $val) {
				...
				if(empty($val[$previous_year]['turnover'])) { $val[$previous_year]['turnover'] = $val[$previous_year]['turnover'] = 0; }
				if(empty($val[$previous_year]['quantity'])) { $val[$previous_year]['quantity'] = $val[$previous_year]['quantity'] = 0; }
	...
	$reports = $coverpage.$contributorspage.$valuesbox.$valuesbox2.$reports.$fxratespage.$closingpage;
	if($core->input['referrer'] != 'generate' && $core->input['referrer'] != 'list' && $core->input['referrer'] != 'direct') {
