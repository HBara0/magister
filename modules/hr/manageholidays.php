...
if(!$core->input['action']) {
	...
	if(isset($core->input['id'])) {
		...
		if($db->num_rows($query) > 0) {
			...
			$holiday['fromDate']= date($core->settings['dateformat'], $holiday['fromDate']);
			$holiday['toDate']= date($core->settings['dateformat'], $holiday['toDate']);
			$action = 'do_edit';
	...
	if($core->input['action'] == 'do_add' || $core->input['action'] == 'do_edit') {
		...
		if(!is_empty($core->input['fromDate'], $core->input['fromTime'], $core->input['toDate'], $core->input['toTime'])) {
			$core->input['fromDate'] = strtotime($core->input['fromDate'].' '.$core->input['fromTime']);
			$core->input['toDate'] = strtotime($core->input['toDate'].' '.$core->input['toTime']);
		}
