...
if(!$core->input['action']) { 
	...
	if(isset($core->input['filtervalue'])) {
		$filter_where = 'AND u.firstname LIKE "%'.$db->escape_string($core->input['filtervalue']).'%"';
		$multipage_where .= 'AND u.firstname LIKE "%'.$db->escape_string($core->input['filtervalue']).'%"';
	}
	...
	if($db->num_rows($query) > 0) {
		while($user = $db->fetch_assoc($query)) {
			...
			$user['positions'] = '';
			$position_query = $db->query("SELECT title FROM ".Tprefix."userspositions up JOIN ".Tprefix."positions p ON (up.posid=p.posid) WHERE up.uid={$user[uid]} ORDER BY title ASC");
