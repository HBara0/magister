 ...
 * Last Update: @zaher.reda		July 11, 2012 | 10:33 AM
 */
class Surveys {
	...
	public function create_survey_template(array $data) {
		...
		echo '<hr />';
			...
			/* Check if same section title has been users in the same template */
			...
			case 'textbox': 
				...
				if(is_array($question['choices']) && !empty($question['choices'])) {
					...
					else
					{
						foreach($question['choices'] as $key => $choice) {
							$question_output .= '<div style="margin-left:20px;">'.$choice.' <input type="text" id="answer_actual_'.$question['stqid'].'_'.$key.'" name="answer[actual]['.$question['stqid'].']['.$key.']" size="'.$question['fieldSize'].'"'.$question_output_inputaccept.' /> <input type="hidden" id="answer_comments_'.$question['stqid'].'_'.$key.'" name="answer[comments]['.$question['stqid'].']['.$key.']" value="'.$key.'"/>'.$this->parse_validation($question).'</div>';
						}
					}
				}
				else /* Single textbox */
				{
					...
					else
					{
						...
						$question_output .= '<div style="margin: 5px 20px; 5px; 20px;"><input type="text" id="answer_'.$question_output_idadd.'_'.$question['stqid'].'" name="answer'.$question_output_idadd.'['.$question['stqid'].']" size="'.$question['fieldSize'].'"'.$question_output_inputaccept.' /> '.$this->parse_validation($question).'</div>';
					}
				}
			break;
			case 'selectlist':
