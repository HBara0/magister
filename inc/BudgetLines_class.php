    ...
    public static function get_budgetline_bydata($data) {
        ...
        if(is_array($data)) {
            ...
            if(isset($data['linkedBudgetLine']) && !empty($data['linkedBudgetLine'])) {
                $budgetline_bydataquery = $db->query("SELECT * FROM ".Tprefix."budgeting_budgets_lines WHERE linkedBudgetLine ='".$data['linkedBudgetLine']."' ANDpid='".$data['pid']."' AND cid='".$data['cid']."' AND altCid='".$db->escape_string($data['altCid'])."' AND saleType='".$data['saleType']."' AND bid='".$data['bid']."' AND customerCountry='".$data['customerCountry']."' AND psid='".$data['psid']."' AND businessMgr='".$data['businessMgr']."'");
            }
            else {
