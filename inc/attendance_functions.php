...
function count_holidays($uid, $check_dates_start, $check_dates_end, $reoccurring_only = false, $specificyear_only = false) {
	...
	if($date_info['start']['year'] == $date_info['end']['year']) {
		...
		$query = $db->query("SELECT * FROM ".Tprefix."holidays WHERE affid='{$user[affid]}' 
			AND ((validFrom = 0 OR ({$date_info[start][year]} >= FROM_UNIXTIME(validFrom, '%Y') AND month >= FROM_UNIXTIME(validFrom, '%m') AND day >= FROM_UNIXTIME(validFrom, '%d'))) AND (validTo=0 OR ({$date_info[end][year]} <= FROM_UNIXTIME(validTo, '%Y') AND month <= FROM_UNIXTIME(validTo, '%m') AND day <= FROM_UNIXTIME(validTo, '%d'))))
...
function parse_additonalfield($attribute, $field_settings) {
	...
	switch($field_settings['type']) {
		case 'inline-search':
			...
			$field = '<input type="text" id="'.$search_for.'_'.$identifier.'_QSearch" value="'.$field_settings['value_attribute_value'].'"/><input type="text" size="3" id="'.$search_for.'_'.$identifier.'_id_output" value="'.$field_settings['key_attribute_value'].'" disabled /><input type="hidden" value="'.$field_settings['key_attribute_value'].'" id="'.$search_for.'_'.$identifier.'_id" name="'.$attribute.'" /><div id="searchQuickResults_'.$identifier.'" class="searchQuickResults" style="display:none;"></div>';
			break;
		case 'select':
			if($field_settings['datasource'] == 'db') {
				if(isset($field_settings['table'], $field_settings['attributes'])) {
					...
					if(is_array($data)) {
						$field = parse_selectlist($attribute, 0, $data, $field_settings['key_attribute_value'], $field_settings['mulitpleselect']);
					}
					else {
