<ul class="list-group">
    ...
    <li class="list-group-item">
        <button type="button" {$status['budget']['disable']} data-toggle="collapse" data-target="#budget" aria-expanded="false" aria-controls="budget" style="background-color: {$status['budget']['color']};height: 50px;width: 100%" class="button">
            {$lang->budget}<span style='height:20px;font-size: 15px;color:{$status['budget']['color']}; background-color: #f8ffcc;text-align: center' class="label label-pill pull-right">{$status['budget']['count']}</span>
        </button>
