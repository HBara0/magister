<html>
<head>
<title>{$core->settings[systemtitle]} | {$lang->edityouraccount}</title>
{$headerinc}
</head>
<body>
{$header}
...
<h3>{$lang->edityouraccount}</h3>
    ...
    <div id='popup_changeprofilepic' title="{$lang->changeprofilepic}">
		...
		<p>
    	 <form action="users.php?action=do_changeprofilepic" method="post" enctype="multipart/form-data" target="uploadFrame">
         	 {$lang->selectprofilepicture} <input type="file" id="uploadfile" name="uploadfile"><br />
