<div id="popup_blacklist" title="{$lang->blacklsitsupplier}" >
    <form name='add_sourcing/managesupplier_Form' id='add_sourcing/managesupplier_Form' method="post">
        ...
        <div style="display:table-cell; vertical-align:middle;">{$lang->daterequested}
            <input type="text" id="pickDate_from" autocomplete="off" tabindex="1" value="{$supplier[details][blacklist][daterequested_output]}" required="required"/>
            <input type="hidden" name="supplier[blacklist][requestedOn]" id="altpickDate_from" value="{$supplier[details][blacklist][fromDate_formatted]}" />
        </div>
        <div class="subtitle">{$lang->selectrequester}</div>
        ...
        <div style="display:table-row">
            <div style="display:table-cell; width:100px; vertical-align:middle;">{$lang->reason}</div>
