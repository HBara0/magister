<div class="container-fluid">
    <form name="perform_portal/conversation_Form" id="perform_portal/conversation_Form"  action="#" method="post">
        ...
        <input type="hidden"  name="conversation[inputChecksum]" value="{$conv_data[inputChecksum]}" />
        <input type="hidden"  name="conversation[scid]" value="{$conv_data[scid]}" />
        {$action}
        <div class="form-group">
            <label for="convtitle">{$lang->title}</label>
            <input {$disableinputs} type="text" class="form-control" id="convtitle" name="conversation[title]" placeholder="{$lang->title}" value='{$conv_data['title']}'>
        </div>
        <div {$hideparticipants}>
