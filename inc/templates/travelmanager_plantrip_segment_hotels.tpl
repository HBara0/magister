<div style="display:block;width:100%;  border:1px solid #FCEFA1;  padding:5px;">
    ...
    <div style="display:inline-block;width:15%;"><input style="width:100px;" id="numnight_segacc_{$approved_hotels[tmhid]}_{$sequence}" accept="numeric" name="segment[{$sequence}][tmhid][{$checksum}][numNights]" type="number" value="{$selectedhotel->numNights}" max="{$leavedays}"> </div>
    ...
    <div id="anotheraff_accomodations_{$sequence}_{$checksum}" style="{$selected_hotel[$sequence][$checksum][displaystatus]} padding: 5px;" class="border_bottom border_left border_right border_top" >
        ...
        <div style="display:inline-block;width:20%;padding:5px;"><input id="affiliate_{$sequence}_{$approved_hotels[tmhid]}_cache_acc_autocomplete" autocomplete="off" tabindex="8" value="{$affiliate->name}" type="text"></div>
