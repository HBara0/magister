<div style="display:block;width:100%;  border:1px solid #FCEFA1;  padding:5px;">
    <div><input aria-describedby="ui-tooltip-155" title="" name="segment[{$sequence}][tmhid][{$checksum}][tmhid]" id="segment[{$sequence}][{$checksum}][tmhid]" value="{$approved_hotels[tmhid]}" type="checkbox" {$hotelchecked}>{$hotel->name} <span>{$review_tools}</span><br />{$hotel->addressLine1}</div>
    <div>
        City: {$cityname}
