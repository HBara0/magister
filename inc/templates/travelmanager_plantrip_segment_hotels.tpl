<div style="display:block;width:100%;  border:1px solid #FCEFA1;  padding:5px;">
    <div class="subtitle" style="  padding-bottom:8px;  {$selectedhotel[$segid][$approved_hotels[tmhid]][hotelsectioncolor]} margin-bottom:10px;">{$selectedhotel[$segid][$approved_hotels[tmhid]][hotelsection]}</div>
    <div >{$checkbox_hotel}<span>{$review_tools}</span></div>
    <div>
        <div style="display:inline-block;padding:5px;width:15%;">{$lang->pricenight}</div>
        <div style="display:inline-block;width:20%;"><input id="pricenight_segacc_{$approved_hotels[tmhid]}_{$sequence}" accept="numeric" name="segment[{$sequence}][tmhid][{$approvedhotel_checksum}][priceNight]" type="text" value="{$selectedhotel[$segid][$approved_hotels[tmhid]][priceNight]}" style="width:100%;"></div>
        <div style="display:inline-block;padding:10px;width:20%;">{$lang->numnight}</div>
        <div style="display:inline-block;width:15%;"><input size="10" id="numnight_segacc_{$approved_hotels[tmhid]}_{$sequence}" accept="numeric" name="segment[{$sequence}][tmhid][{$approvedhotel_checksum}][numNights]" type="text" value="{$selectedhotel[$segid][$approved_hotels[tmhid]][numNights]}"> </div>
    ...
    <div id="anotheraff_accomodations_{$sequence}_{$approved_hotels[tmhid]}" style="{$selectedhotel[$segid][display]} padding: 5px;" class="border_bottom border_left border_right border_top" >
        ...
        <input id="affiliate_{$sequence}_{$approved_hotels[tmhid]}_cache_acc_id" name="segment[{$sequence}][tmhid][{$approvedhotel_checksum}][paidBy]"    value="{$selectedhotel[$segid][$approved_hotels[tmhid]][affid]}" type="hidden">
    </div>
