...
$(function(){
	...
	$(".pricenotOk").live('change',function() {
		...
		$(".notapproved").live('change',function() {
			$(this).parent().parent().nextAll().has("textarea,:text,:radio,:checkbox").first().find("textarea,:text,:radio,:checkbox").attr( 'disabled', true); 
		});
		
	$(".approved").live('change',function() {
		/* find the first checkbox in the next parent div after each input with class approved*/
		var  obj = $(this).parent().parent().nextAll().has(":checkbox").first().find("textarea,:text,:radio,:checkbox").removeAttr( "disabled" ).prop("checked",true); 
	...
	$(".stageapproved").live('change',function() {
		...
		var  obj = $(this).parent().parent().parent().nextAll().has(":checkbox").first().find("textarea,:text,:radio,:checkbox").removeAttr( "disabled" ).prop("checked",true);
		$("div[id^='" + obj.val() + "']").show(); /* obj.val() Get the value of the checkbox in the next div (that has calss main) */
	});

	$(".stagenotapproved").live('change',function() {
		$("html, body").animate({ scrollTop: $(document).height() }, "slow");  /*scroll down to the end of body */
		$(this).parent().parent().parent().nextAll().has(":checkbox").first().find("textarea,:text,:radio,:checkbox").attr( 'disabled', true); 
