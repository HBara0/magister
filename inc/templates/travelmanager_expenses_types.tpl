<tr class="{$altrow}"  style="border:1px gainsboro solid;"id="{$sequence}_{$rowid}"><td>
        ...
        <div style="display:inline-block;padding:5px;width:70%;"><select id="segment_expensestype_{$sequence}_{$rowid}" name='segment[{$sequence}][expenses][{$rowid}][tmetid]'   {$onchange_actions}>{$expenses_options}</select></div>
