...
Abstract class AbstractClass {
    ...
    public function save(array $data = array()) {
        ...
        if(isset($this->data['inputChecksum']) && !empty($this->data['inputChecksum'])) {
            $object = self::get_data(array('inputChecksum' => $this->data['inputChecksum']));
            if(is_object($object)) {
                return $object->update($data);
            }
        }

        if(is_array(static::UNIQUE_ATTRS)) {
            foreach(static::UNIQUE_ATTRS as $attr) {
                if(empty($data[$attr])) {
                    $checks = null;
                    break;
                }
