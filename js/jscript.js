$(function() {
	...
	*/
    ...
    $(document).on("click", "input[id^='perform_'][id$='_Button'],input[id^='add_'][id$='_Button'],input[id^='change_'][id$='_Button']", function(e) {
    ...
    $('input[title],a[title],div[title]').qtip({
        style: {
    ...
    $("img[id^='ajaxaddmore_']").live("click", function() {
        ...
        var id = $(this).attr('id').split('_');
        alert(id);
        var section_id =  $("#sectionvalue").val();
        var sections_count = parseInt(section_id)+1;
        var subsection_id = $("#section"+section_id+"_subsection").val();
        var subsection_count = parseInt(subsection_id)+1;
        alert('subsection_id  '+subsection_id);
        alert('section_id  '+section_id);
        //alert(id[1] +"&action=ajaxaddmore_"+id[2]);
        ...
        function(returnedData) {
            alert(returnedData);
            //  alert('which section'+$('#'+id[2]+'_'+section_id+'_tbody').html());
            $('#'+id[2]+'_'+id[3]+'_tbody').append(returnedData);
            $("#subsection_id").val(subsection_count);
    ...
    window.sharedFunctions = function() {
        ...
        function addmoreRows(trigger) {
            ...
            //$("input[id='sequence_"+id[2]+"_"+questionincrement+"']")
          
            var questionincrement = parseInt(last) ; 
            
            if($("input[id='sequence_"+id[2]+"_"+questionincrement+"']").length > 0){ 
                $("#sequence_"+id[2]+"_"+questionincrement).attr("disabled",true).css('background-color', '#F2F2F2').css('font-weight', 'bold');
                $("#sequence_"+id[2]+"_"+questionincrement).val(questionincrement);
            }
