 ...
 * Last Update: @zaher.reda		June 18, 2012 | 01:02 PM
 */
	...
	$(".calendar_hour").live('mouseenter', function() {
		...
		$("#weekview_table").selectable({
			...
			stop: function(event, ui) {
				...
				$(".ui-dialog, #popup_weekview_createentry").css("overflow", "visible");
			}
		});
	});
	...
	$(".calendar_hourevent").live('mouseover', function() {
		...
		$(this).draggable({ 
			...
			stop: function(event, ui) {
				...
				$.post("index.php?module=calendar/weekviewoperations&action=update_time", { todiff: diff, fromdiff: diff, id: id[1]}, function(returnedData) { if(returnedData == '0') { $('#leave_'+id[1]).animate({top: originalTop, left: originalLeft}, 'slow'); }});		
	...
	$(".calendar_hourevent").live('mouseover', function() {
		...
		$(this).resizable({
			...
			stop: function(event, ui) {
				...
				$.post("index.php?module=calendar/weekviewoperations&action=update_time", { todiff: diff, id: id[1] }, function(returnedData) { if(returnedData == '0') { $('#leave_'+id[1]).animate({height: ui.originalSize.height}, 'fast'); $('#toTime_'+id[1]).text(originalTime[0]+':'+originalTime[1]); }});	
			}
		});
	});
