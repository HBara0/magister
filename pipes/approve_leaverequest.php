...
if(strpos($data['message'], '#approve')) {
    echo 'approve';
    ...
    $request_key = $db->escape_string('19872bfc12');
    //$leave = $db->fetch_assoc($db->query("SELECT l.*, u.firstName, u.lastName, email FROM ".Tprefix."leaves l LEFT JOIN ".Tprefix."users u ON (u.uid=l.uid) WHERE l.requestKey='{$request_key}'"));

    $leave_obj = new Leaves(array('lid' => 105));
...
// get leave message  of the current leave
    $leavemessage_obj = new LeavesMessages();
    $leavemessage_obj->create_message($data[message]);
