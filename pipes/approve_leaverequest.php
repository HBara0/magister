	...
	if($db->num_rows($query) > 0) {
		...
		if($db->affected_rows() > 0) {
			$query3 = $db->query("SELECT l.uid, u.email 
								...
								WHERE l.isApproved='0' AND l.lid='{$leave[lid]}' AND sequence=((SELECT sequence FROM ".Tprefix."leavesapproval WHERE lid='{$leave[lid]}' AND uid='{$user[uid]}' AND isApproved='1')+1)");
