...
elseif($core->input['action'] == 'do_create') {
    ...
    $function_obj = $function_obj->save($core->input['chemicalfunctions']);
    switch($function_obj->get_errorcode()) {
        case 0:
            if(is_array($core->input['lang'])) {
                $trans_data['tableKey'] = intval($function_obj->{ChemicalFunctions::PRIMARY_KEY});
                $trans_data['tableName'] = ChemicalFunctions::TABLE_NAME;
...
elseif($core->input['action'] == 'get_updatefunction') {
    ...
    if(is_array($languages)) {
        foreach($languages as $language) {
            ...
            eval("\$translationtabs_content=\"".$template->get('admin_products_translatefunction_content')."\";");
            $lang_output .= $translationtabs_content;
            $lid = '';
        }
    }
    eval("\$translationsprodfunc = \"".$template->get('admin_translationsprodfunc')."\";");
