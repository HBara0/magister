 ...
 * $module: admin/home
 * $id: stats.php	
 * Last Update: @najwa.kassem 	July 05, 2011 | 11:35 AM
 */
if(!$core->input['action']) {
	...
	$ocosusers = $db->query("SELECT COUNT(*) as countall, affid FROM ".Tprefix."affiliatedemployees ae JOIN ".Tprefix."users u ON (u.uid=ae.uid) WHERE isMain=1 AND gid!=7 GROUP BY affid");
	while($users = $db->fetch_assoc($ocosusers)){
		$stats['ocosusers'] .= '<li>'.$users['countall'].' employees in '.$affiliate_name[$users['affid']].'</li>';
	}
