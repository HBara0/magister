...
if($_REQUEST['authkey'] == 'kia5ravb$op09dj4a!xhegalhj') {
    ...
    $currentcountry = 0;
    $entities = Entities::get_data('isActive =1 AND type = "c" AND eid IN (SELECT eid FROM '.EntitiesSegments::TABLE_NAME.' WHERE psid IN(24,4)) ORDER BY country', array('operators' => array('filter' => 'CUSTOMSQLSECURE')));
    if(is_array($entities)) {
        ...
        $message.='<table width="100%" cellspacing="0" cellpadding="5" style="border-left: 1px solid #CCC;" border="0"><thead><tr style="background-color:#EAEAEA"> <th style="border: 1px solid black;">Representative Name</th><th style="border: 1px solid black;">Representative Email</th><th style="border: 1px solid black;">Customer Name</th><th style="border: 1px solid black;">Country</th><th style="border: 1px solid black;">Segment</th></tr></thead><tbody>';
        foreach($entities as $entity) {
            ...
            if(is_array($representatives)) {
                ...
                if(is_array($entsegments_objs)) {
                    if(empty($currentcountry) || $currentcountry != $entity->country) {
                        $currentcountry = $entity->country;
                        $message.='<tr><td colspan="5"><hr></td></tr>';
                        $message.='<tr><td colspan="5" style="text-align:center"><h2>'.$country.'</h2></td></tr>';
                        $message.='<tr><td colspan="5"><hr></td></tr>';
                    }
                ...
                foreach($representatives as $representative) {
                    $message.="<tr><td style='border: 1px solid black;'>{$representative->get_displayname()}</td><td style='border: 1px solid black;'>{$representative->email}</td><td style='border: 1px solid black;'>{$entity->get_displayname()}</td>"
                            ."<td style='border: 1px solid black;'>{$country}</td><td style='border: 1px solid black;'>{$segments}</td></tr>";
                }
            }
            unset($entsegments_objs, $segments_list, $segments_obj, $segments, $country);
        }
